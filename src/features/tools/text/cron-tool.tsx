    ...
    <div className="space-y-5">
      <label className="grid gap-2 text-sm font-medium">
        ...
        <input
          ...
          spellCheck={false}
          className="h-12 rounded-xl border border-border bg-background px-4 font-mono"
        />
      </label>
      <div className="grid grid-cols-5 gap-2 text-center text-xs text-muted-foreground">
        {(zh
          ...
          <span key={label} className="rounded-lg bg-background-muted p-2">
            {label}
          </span>
        ))}
      </div>
      {result.error ? (
        ...
        <p className="rounded-2xl border border-border bg-background p-5 leading-7">
          {result.explanation}
        </p>
      )}
      <p className="text-xs text-muted-foreground">
        {zh
          ? '按标准 5 段 Cron 解析，不支持 Quartz 的秒、年份、?、L、W、# 等扩展语法。'
          : 'Uses standard five-field cron syntax and intentionally does not accept Quartz-specific seconds, year, ?, L, W, or # extensions.'}
      </p>
    </div>
  )
}
