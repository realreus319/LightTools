  ...
  const decode = () => {
    ...
    try {
      ...
      try {
        ...
        setOutput(zh ? '二进制数据：请使用“下载解码文件”。' : 'Binary data: use “Download decoded file”.')
      }
    } catch (value) {
      setError(value instanceof Error ? value.message : String(value))
    }
    ...
    <div className="space-y-5">
      {mode === 'hash' ? (
        <label className="grid max-w-xs gap-2 text-sm font-medium">
          ...
          <select value={algorithm} onChange={(event) => setAlgorithm(event.currentTarget.value as HashAlgorithm)} className="h-10 rounded-xl border border-border bg-background px-3">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <label className="grid gap-2 text-sm font-medium">
          ...
          <textarea value={input} onChange={(event) => setInput(event.currentTarget.value)} spellCheck={false} className="min-h-64 rounded-2xl border border-border bg-background p-4 font-mono text-sm" />
        </label>
        <label className="grid gap-2 text-sm font-medium">
      ...
      {mode === 'base64' ? (
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" onClick={decode}>{zh ? '解码文本' : 'Decode text'}</Button>
          <label className="inline-flex cursor-pointer items-center rounded-xl border border-border px-4 py-2 text-sm font-medium">
            ...
            <input type="file" className="sr-only" onChange={(event) => {
              ...
              if (!file) return
              void file.arrayBuffer().then((buffer) => setOutput(bytesToBase64(new Uint8Array(buffer))))
              event.currentTarget.value = ''
            }} />
          </label>
          {decodedBytes ? <Button variant="outline" onClick={() => downloadBlob(new Blob([decodedBytes.buffer]), 'lighttools-decoded.bin')}>{zh ? '下载解码文件' : 'Download decoded file'}</Button> : null}
        </div>
      ) : (
        <div className="flex flex-wrap gap-2">
          <Button onClick={() => void digest(new TextEncoder().encode(input).buffer)}>{zh ? '计算文本 Hash' : 'Hash text'}</Button>
          <label className="inline-flex cursor-pointer items-center rounded-xl border border-border px-4 py-2 text-sm font-medium">
