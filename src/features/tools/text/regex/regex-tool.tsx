    ...
    try {
      ...
      setError(
        value instanceof ToolError
          ? getToolErrorMessage(locale, value.code)
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-4 sm:grid-cols-[1fr_10rem]">
        <label className="grid gap-2 text-sm font-medium">
          ...
          <input
            ...
            className="h-10 rounded-xl border border-border bg-background px-3 font-mono"
          />
        </label>
        <label className="grid gap-2 text-sm font-medium">
          <span>Flags</span>
      ...
      <p className="text-xs text-muted-foreground">
        {zh
          ? '表达式在独立 Worker 中执行，超过时间预算会直接终止 Worker，避免卡死页面。'
          : 'The expression runs in an isolated worker that is terminated if it exceeds the time budget.'}
      </p>
      {error ? (
      ...
      <div className="space-y-2">
        {matches.map((match, index) => (
          <div
            ...
            className="rounded-xl border border-border p-3 font-mono text-sm"
          >
            <span className="text-muted-foreground">@{match.index}</span> {match.value || '∅'}
            {match.groups.length ? (
              <span className="ml-2 text-muted-foreground">[{match.groups.join(', ')}]</span>
            ) : null}
          </div>
        ))}
      </div>
