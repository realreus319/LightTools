    ...
    <div className="space-y-5">
      {mode === 'uuid' ? (
        ...
        <label className="grid gap-2 text-sm font-medium">
          ...
          <textarea
            ...
            spellCheck={false}
            className="min-h-40 rounded-2xl border border-border bg-background p-4 font-mono text-sm"
          />
        </label>
      )}
      ...
      {mode === 'jwt-decode' ? (
        <p className="rounded-xl bg-background-muted p-3 text-sm text-muted-foreground">
          {zh
            ? '这里只解析 Header 与 Payload，不验证签名，也不代表 Token 可信。'
            : 'This only decodes the header and payload. It does not verify the signature or establish trust.'}
        </p>
      ) : null}
