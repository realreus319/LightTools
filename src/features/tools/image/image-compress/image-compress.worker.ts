...
function parsePayload(value: unknown): ImageProcessPayload {
  ...
  if (!(payload.buffer instanceof ArrayBuffer))
    throw new TypeError('Image payload is missing buffer')
  if (!payload.inputMime || !isSupportedImageMime(payload.inputMime))
    throw new TypeError('Unsupported input mime')
  if (!payload.outputMime || !isSupportedImageMime(payload.outputMime))
    throw new TypeError('Unsupported output mime')
