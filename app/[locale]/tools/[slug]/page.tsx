...
export default async function ToolPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const toolContent =
    tool.slug === 'image-compress' ? <ImageCompressTool locale={locale} /> :
    tool.slug === 'image-convert' ? <ImageTransformTool locale={locale} mode="convert" /> :
    tool.slug === 'image-resize' ? <ImageTransformTool locale={locale} mode="resize" /> :
    tool.slug === 'image-metadata-remove' ? <ImageTransformTool locale={locale} mode="metadata" /> : undefined

  return (
    <>
      <a href="#main-content" className="lt-skip-link">{copy.title}</a>
      <SiteHeader locale={locale} messages={messages} languageHref={`/${alternateLocale}/tools/${tool.slug}`} />
      <ToolPageShell locale={locale} messages={messages} tool={tool} slug={tool.slug as ToolSlug} title={copy.title} description={copy.description}>
        {toolContent}
      </ToolPageShell>
        ...
        <Container>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {related.map((relatedTool) => {
              ...
              return <ToolCard key={relatedTool.slug} href={`/${locale}/tools/${relatedTool.slug}`} title={relatedCopy.title} description={relatedCopy.description} category={messages.categories[relatedTool.category].title} status={relatedTool.status} labels={messages.toolCard} localOnly={relatedTool.localOnly} />
            })}
          </div>
        </Container>
      </section>
      <section className="border-t border-border/60 bg-background-muted/40 py-12">
        <Container>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">{messages.toolPage.privacyDescription}</p>
        </Container>
      </section>
